...
def autodetect_port():
    ...
    for p in ports:
        # macOS / Linux / Windows CDC
        if "usbmodem" in p.device or "ttyACM" in p.device:
            return p.device
        # No Windows: procura dispositivos USB Serial / Seeed / J-Link
        if "COM" in p.device and ("USB" in (p.description or "") or "Serial" in (p.description or "")):
            return p.device
    # Se só houver uma porta COM disponível e não for a COM1 (que costuma ser interna), usa essa:
    for p in ports:
